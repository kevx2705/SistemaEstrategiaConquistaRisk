...
/**
 * Servicio que gestiona la lógica de las partidas del juego Risk.
 * Proporciona funcionalidades para crear, inicializar, retomar y finalizar partidas,
 * así como gestionar las fases del juego, los movimientos de tropas y el canje de cartas.
 */
@Service
public class PartidaService {
    ...
    /**
     * Constructor de la clase {@code PartidaService}.
     * Inicializa la lista de movimientos para la fase 3.
     */
    ...
    /**
     * Permite a un jugador reclamar un territorio durante la fase de inicialización.
     *
     * @param partidaId    Identificador de la partida.
     * @param jugadorId    Identificador del jugador.
     * @param territorioId Identificador del territorio a reclamar.
     * @throws RuntimeException Si no es el turno del jugador, el territorio no existe o ya está asignado.
     */
    @Transactional
     ...
     * @throws RuntimeException Si la cantidad es inválida, el territorio no existe o no pertenece al jugador.
     */
     ...
     * @throws RuntimeException Si no es el turno del jugador, los territorios no existen o no están conectados.
     */
    @Transactional
    public void moverTropasFase3(Long partidaId, Long jugadorId, String territorioOrigen, String territorioDestino, int cantidad) {
        Partida partida = partidaRepository.findById(partidaId)
                .orElseThrow(() -> new RuntimeException("No existe la partida"));
        if (!partida.getJugadorActualId().equals(jugadorId)) {
     ...
     * @throws RuntimeException Si la partida no existe, el jugador no está en la partida o la combinación es inválida.
     */
    @Transactional
    public int canjearCartas(Long partidaId, Long jugadorId, MyLinkedList<Long> idsCartas) {
        Partida partida = partidaRepository.findById(partidaId)
                .orElseThrow(() -> new RuntimeException("Partida no existe"));
        MyLinkedList<Long> listaIds = gson.fromJson(partida.getJugadoresOrdenTurnoJSON(), new TypeToken<MyLinkedList<Long>>() {}.getType());
     ...
     * @param territorios Lista de territorios.
     * @return {@code TerritorioDTO} Territorio libre encontrado, o null si no hay territorios libres.
     */
     ...
     * @return {@code MyLinkedList<Long>} Lista enlazada con los identificadores de los jugadores.
     ...
     * @return {@code MyLinkedList<JugadorDTO>} Lista enlazada de jugadores en formato DTO.
     */
    ...
    public void eliminarJugador(Long partidaId, Long jugadorId) {
        Partida partida = partidaRepository.findById(partidaId)
                .orElseThrow(() -> new RuntimeException("Partida no existe"));
        MyLinkedList<JugadorDTO> jugadores = gson.fromJson(partida.getJugadoresOrdenTurnoJSON(), new TypeToken<MyLinkedList<JugadorDTO>>() {}.getType());
        MyLinkedList<TerritorioDTO> territorios = gson.fromJson(partida.getTerritoriosJSON(), new TypeToken<MyLinkedList<TerritorioDTO>>() {}.getType());
        Node<JugadorDTO> nodoJugador = getNodoPorId(jugadores, jugadorId);
        if (nodoJugador == null) {
    ...
    private MyLinkedList<TerritorioDTO> cargarTerritorios(Partida partida) {
        if (partida.getTerritoriosJSON() != null && !partida.getTerritoriosJSON().isBlank()) {
            return gson.fromJson(partida.getTerritoriosJSON(), new TypeToken<MyLinkedList<TerritorioDTO>>() {}.getType());
        }
     ...
     * @param jugadorId Identificador del jugador.
     * @param partidaId Identificador de la partida.
     * @return boolean Verdadero si el jugador ya movió tropas, falso en caso contrario.
     */
     ...
     * @return {@code Long} Identificador del ganador, o null si la partida no ha finalizado.
     */
    ...
    public PartidaDTO retomarPartida(String codigoHash) {
        ...
        if (partida.getMazoCartasJSON() != null && !partida.getMazoCartasJSON().isBlank()) {
            Type mazoType = new TypeToken<MyLinkedList<CartaDTO>>() {}.getType();
     ...
     * @return {@code MyLinkedList<Jugador>} Lista enlazada de jugadores de la partida.
     */
     ...
     *
     * @return {@code Long} Identificador de la partida actual, o null si no hay ninguna.
     */
     ...
     * @throws RuntimeException Si la partida no existe o la lista de jugadores está vacía.
     */
